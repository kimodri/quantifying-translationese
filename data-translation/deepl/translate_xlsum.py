#!/usr/bin/env python3
"""Translate cleaned XLSum CSV using DeepL API.

Example:
  python data-translation/deepl/translate_xlsum.py \
    --input datasets/cleaned/cleaned_xlsum.csv \
    --output datasets/translated/deepl/deepl_translated_xlsum.csv \
    --target-lang TL
...
def main():
    ...
    parser.add_argument('--input', default='datasets/cleaned/cleaned_xlsum.csv')
    parser.add_argument('--output', default='datasets/translated/deepl/deepl_translated_xlsum.csv')
    parser.add_argument('--columns', default='text,summary', help='Comma-separated columns to translate')
    parser.add_argument('--target-lang', default='TL', help='DeepL target language code (e.g., TL for Tagalog).')
    parser.add_argument('--batch-size', type=int, default=10, help='Number of texts per DeepL request')
